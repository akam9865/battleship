...
function createAIGame (user) {
	...
	user.on('HIT', function (data) {
		// add data's neighbors to targets list.
  ...
  user.on("MESSAGE", function (data) {
    user.emit("MESSAGE", data);
		setTimeout(function () {
			user.emit("MESSAGE", {username: "Ava", message: "I'm afraid I can't do that"});
		}, 1000);
  })
};
...
AIGame.prototype = {
	...
	shot: function () {
		// choose from this.targets or call randomShot
